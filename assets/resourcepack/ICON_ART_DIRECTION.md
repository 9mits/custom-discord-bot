...
| `mythic_giftbag` | Full royal-purple drawstring gift pouch with a broad contiguous body, simple antique-gold cord, and one amethyst clasp; no detached detail or crushed shadows. |
...
They are exact 64x64 source assets rather than generated substitutes. These three
icons are the only exception to the shared 18x18 import grid: never resample,
re-light, quantize, trim, or otherwise pixelize them. They are locked out of
`import_generated_icons.py` so future bulk imports preserve every supplied pixel.
The placement order is cyan, violet, then dark silver. Java and Bedrock resolve the
same three canonical PNG files.
