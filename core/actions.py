...
ACTION_SPECS: tuple[ActionSpec, ...] = (
    ...
    _spec("userinfo", "Information", "View account, join, and role details for a member.", ack=AcknowledgementPolicy.DEFER, examples=("/userinfo", "/userinfo user:@member"), related=("history", "avatar")),
    _spec("avatar", "Information", "View a member's full-size avatar.", examples=("/avatar user:@member",), related=("userinfo",)),
    _spec("roleinfo", "Information", "View permissions and membership for a role.", examples=("/roleinfo role:@Staff",), related=("serverinfo",)),
    _spec("channelinfo", "Information", "View details for a channel.", examples=("/channelinfo",), related=("serverinfo",)),
    _spec("ping", "Information", "Check whether the bot is responsive.", examples=("/ping",), related=("status", "about")),
    _spec("timestamp", "Information", "Turn a duration from now into Discord timestamp codes.", examples=("/timestamp duration:2h30m",), related=("endtimestamp",)),
    _spec("slowmode", "Moderation", "Set or clear the slowmode delay on a channel.", capability="channels.lock", caller=_STAFF, bot=("manage_channels",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.CONFIGURATION, examples=("/slowmode seconds:10",), related=("lock",)),
    _spec("nickname", "Moderation", "Change or reset a member's nickname.", capability="cases.read", caller=_STAFF, bot=("manage_nicknames",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.MODERATE, examples=("/nickname user:@member",), related=("userinfo",)),
    _spec("poll", "Information", "Post a reaction poll with up to ten choices.", capability="cases.read", caller=_STAFF, bot=("send_messages", "add_reactions"), ack=AcknowledgementPolicy.DEFER, examples=("/poll question:Ready? options:Yes | No",)),
    _spec("note add", "Cases", "Record a private staff note about a member.", capability="cases.read", caller=_STAFF, ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.MODERATE, examples=("/note add user:@member content:Context",), related=("note list", "history")),
    _spec("note list", "Cases", "View private staff notes about a member.", capability="cases.read", caller=_STAFF, ack=AcknowledgementPolicy.DEFER, examples=("/note list user:@member",), related=("note add", "history")),
    _spec("note remove", "Cases", "Delete a staff note by its id.", capability="cases.read", caller=_STAFF, ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.DESTRUCTIVE, examples=("/note remove note:12",), related=("note list",)),
