...
ACTION_SPECS: tuple[ActionSpec, ...] = (
    _spec("settings", "Configuration", "Open the unified server settings hub.", capability="config_panel", caller=_ADMIN, feature="config_panel", examples=("/settings",), related=("setup", "config")),
    ...
    _spec("punish", "Moderation", "Open the punishment workflow for a member.", capability="punishments.issue", caller=_STAFF, bot=("moderate_members", "kick_members", "ban_members"), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.DESTRUCTIVE, examples=("/punish user:@member", "/punish userid:123456789"), related=("history", "undo", "rules")),
    _spec("publicexecution", "Moderation", "Start a community-vote punishment.", capability="punishments.issue", caller=_STAFF, risk=RiskLevel.DESTRUCTIVE, related=("punish",)),
    _spec("history", "Cases", "View a member's moderation history.", capability="cases.read", caller=_STAFF, examples=("/history user:@member",), related=("cases", "case")),
    ...
    _spec("case", "Cases", "Open one moderation case.", capability="cases.read", caller=_STAFF, examples=("/case caseid:42",), related=("cases", "undo")),
    _spec("undo", "Moderation", "Reverse a logged moderation action.", capability="punishments.undo", caller=_STAFF, risk=RiskLevel.DESTRUCTIVE, examples=("/undo user:@member",), related=("case", "history")),
    _spec("purge", "Moderation", "Delete recent messages with optional filters.", capability="messages.purge", caller=_STAFF, bot=("manage_messages", "read_message_history"), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.DESTRUCTIVE, examples=("/purge amount:50",), related=("export",)),
    ...
    _spec("directory", "Analytics", "View the staff directory.", capability="cases.read", caller=_STAFF, related=("stats",)),
    ...
    _spec("serverinfo", "Information", "View detailed server information.", related=("about",)),
    ...
    _spec("unarchive", "Channels", "Restore an archived channel.", capability="channels.lock", caller=_STAFF, bot=("manage_channels",), risk=RiskLevel.CONFIGURATION, related=("archive",)),
    _spec("clone", "Channels", "Archive a channel and create a replacement.", capability="channels.lock", caller=_STAFF, bot=("manage_channels",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.DESTRUCTIVE, related=("archive",)),
    ...
    _spec("derole", "Custom Roles", "Remove selected roles from every member who has them.", capability="config_panel", caller=_STAFF, bot=("manage_roles",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.DESTRUCTIVE, related=("role-settings",)),
    _spec("event setup", "Events", "Choose where the event leaderboard is displayed.", caller=_ADMIN, risk=RiskLevel.CONFIGURATION),
    _spec("event start", "Events", "Start or resume event voice tracking.", caller=_ADMIN, risk=RiskLevel.CONFIGURATION),
    _spec("event stop", "Events", "Pause event voice tracking.", caller=_ADMIN, risk=RiskLevel.CONFIGURATION),
    _spec("event reset", "Events", "Reset tracked event time.", caller=_ADMIN, risk=RiskLevel.DESTRUCTIVE),
    _spec("event goal", "Events", "Set the event target in hours.", caller=_ADMIN, risk=RiskLevel.CONFIGURATION),
    _spec("event next", "Events", "Archive the event and begin another.", caller=_ADMIN, risk=RiskLevel.DESTRUCTIVE),
    _spec("event vc", "Events", "Choose the tracked voice channel.", caller=_ADMIN, risk=RiskLevel.CONFIGURATION),
    _spec("event elapsed", "Events", "Correct the event elapsed time.", caller=_ADMIN, risk=RiskLevel.CONFIGURATION),
    _spec("event status", "Events", "Show event configuration and runtime state.", caller=_ADMIN),
    _spec("endtimestamp", "Events", "Show Discord timestamp formats for the projected event end.", caller=_ADMIN),
    _spec("Add to Scam Image Dataset", "Context Actions", "Add images from a message to the scam dataset.", capability="config_panel", caller=_STAFF, risk=RiskLevel.CONFIGURATION),
    _spec("Punish Message", "Context Actions", "Punish a message author and retain evidence.", capability="punishments.issue", caller=_STAFF, risk=RiskLevel.DESTRUCTIVE),
    _spec("Moderation History", "Context Actions", "View a selected member's case history.", capability="cases.read", caller=_STAFF),
)
