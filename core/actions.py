...
ACTION_SPECS: tuple[ActionSpec, ...] = (
    ...
    _spec("reset-server", "Security", "Permanently clear server content and bans while preserving members.", capability="server.reset", caller=("Discord Administrator",), bot=("administrator",), risk=RiskLevel.DESTRUCTIVE, related=("kick-all-members", "destroy-server")),
    _spec("kick-all-members", "Security", "Kick every other removable member and bot while preserving the runner.", capability="server.reset", caller=("Discord Administrator",), bot=("administrator",), risk=RiskLevel.DESTRUCTIVE, related=("reset-server", "destroy-server")),
    _spec("destroy-server", "Security", "Erase server content and bans, kick members, then remove the bot.", capability="server.reset", caller=("Discord Administrator",), bot=("administrator",), risk=RiskLevel.DESTRUCTIVE, related=("reset-server", "kick-all-members")),
    ...
    _spec("servertag role", "Custom Roles", "Set the role given to server tag wearers.", capability="config_panel", caller=_ADMIN, bot=("manage_roles",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.CONFIGURATION, related=("servertag link", "servertag status")),
    _spec("servertag link", "Custom Roles", "Also accept another server's tag.", capability="config_panel", caller=_ADMIN, bot=("manage_roles",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.CONFIGURATION, related=("servertag role", "servertag unlink")),
    _spec("servertag unlink", "Custom Roles", "Stop accepting another server's tag.", capability="config_panel", caller=_ADMIN, bot=("manage_roles",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.CONFIGURATION, related=("servertag link",)),
    _spec("servertag sync", "Custom Roles", "Re-check every member's server tag right now.", capability="config_panel", caller=_ADMIN, bot=("manage_roles",), ack=AcknowledgementPolicy.DEFER, risk=RiskLevel.CONFIGURATION, related=("servertag role", "servertag status")),
    _spec("servertag status", "Custom Roles", "Show the current server tag setup.", capability="config_panel", caller=_ADMIN, related=("servertag role",)),
